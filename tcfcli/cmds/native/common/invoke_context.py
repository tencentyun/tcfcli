...
class InvokeContext(object):
    ...
    def _get_namespace(self, resource):
        ...
        if self._namespace:
            ...
            for n in resource.keys():
                ns = resource.get(n, None)
    ...
    def _get_function(self, namespace):
        ...
        if self._function:
            ...
            for f in namespace.keys():
                fun = namespace.get(f, None)
        if not fun:
    ...
    def __enter__(self):
        ...
        self.runtime = Runtime(func[model.PROPERTY])
        self.debug_context = DebugContext(self._debug_port, self._debug_argv, self.runtime.runtime)
        return self
    ...
    @property
    def argv(self):
        argv = self.debug_context.argv
        runtime_pwd = os.path.dirname(os.path.abspath(__file__))
        bootstrap = os.path.join(runtime_pwd, "runtime", self.runtime.runtime,
                            self.BOOTSTRAP_SUFFIX[self.runtime.runtime])
        code = os.path.normpath(
            os.path.join(os.path.dirname(os.path.abspath(self._template_file)), self.runtime.codeuri))
    ...
    def env(self):
        env = {
            'SCF_LOCAL': 'true',
            'SCF_FUNCTION_MEMORY_SIZE': str(self.runtime.mem_size),
            'SCF_FUNCTION_TIMEOUT': str(self.runtime.timeout),
            'SCF_EVENT_BODY': self._event,
            'SCF_FUNCTION_ENVIRON': json.dumps(self.runtime.env)
        }
    ...
    def get_handler(self):
        res = self.runtime.handler.split('.', 1)
        if len(res) > 1:
            return res[0] + ':' + res[1]
        return res[0]